class ProdectsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @prodect.save
        format.html { redirect_to prodect_url(@prodect), notice: "Prodect was successfully created." }
  ...
  def update
    respond_to do |format|
      if @prodect.update(prodect_params)
        format.html { redirect_to prodect_url(@prodect), notice: "Prodect was successfully updated." }
  ...
  private
    ...
    def prodect_params
      params.require(:prodect).permit(:title, :description, :image_url, :price)
    end
end
